...
public class Shrine : Destination
{
    // Shrine fufills the belief needs of the gnomes after a collision
        ...
        if (collison != null)
        {
            if (collison.CompareTag("NPC"))
            {
                ...
                if (gnomeAI.GetDest() == gameObject)
                {
                    ...
                    {
                        gnomeAI.GetReligiousDecision().UpdateNeed(-m_updateAmmount);
                        gnomeStats.UpdateSkillLevel(gnomeAI.m_belief, 0.1);
                    }
                }
            }
